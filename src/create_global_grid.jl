...
Construct and return a Cartesian grid of MPI processes, defining implicitely a global grid.
...
function create_global_grid(nx::Integer, ny::Integer, nz::Integer; dimx::Integer=0, dimy::Integer=0, dimz::Integer=0, periodx::Integer=0, periody::Integer=0, periodz::Integer=0, overlaps::Tuple{Int,Int,Int}=(2, 2, 2), halowidths::Tuple{Int,Int,Int}=max.(1, overlaps .÷ 2), disp::Integer=1, reorder::Integer=1, comm::MPI.Comm=MPI.COMM_WORLD, device_type::String=DEVICE_TYPE_AUTO, select_device::Bool=true, quiet::Bool=false)
    ...
    if haskey(ENV, "IGG_CUDAAWARE_MPI")
        cudaaware_MPI .= (parse(Int64, ENV["IGG_CUDAAWARE_MPI"]) > 0)
    end
    if haskey(ENV, "IGG_ROCMAWARE_MPI")
        amdgpuaware_MPI .= (parse(Int64, ENV["IGG_ROCMAWARE_MPI"]) > 0)
    end
    if haskey(ENV, "IGG_USE_POLYESTER")
        use_polyester .= (parse(Int64, ENV["IGG_USE_POLYESTER"]) > 0)
    end
    if none(cudaaware_MPI)
        if haskey(ENV, "IGG_CUDAAWARE_MPI_DIMX")
            cudaaware_MPI[1] = (parse(Int64, ENV["IGG_CUDAAWARE_MPI_DIMX"]) > 0)
        end
        if haskey(ENV, "IGG_CUDAAWARE_MPI_DIMY")
            cudaaware_MPI[2] = (parse(Int64, ENV["IGG_CUDAAWARE_MPI_DIMY"]) > 0)
        end
        if haskey(ENV, "IGG_CUDAAWARE_MPI_DIMZ")
            cudaaware_MPI[3] = (parse(Int64, ENV["IGG_CUDAAWARE_MPI_DIMZ"]) > 0)
        end
    end
    if none(amdgpuaware_MPI)
        if haskey(ENV, "IGG_ROCMAWARE_MPI_DIMX")
            amdgpuaware_MPI[1] = (parse(Int64, ENV["IGG_ROCMAWARE_MPI_DIMX"]) > 0)
        end
        if haskey(ENV, "IGG_ROCMAWARE_MPI_DIMY")
            amdgpuaware_MPI[2] = (parse(Int64, ENV["IGG_ROCMAWARE_MPI_DIMY"]) > 0)
        end
        if haskey(ENV, "IGG_ROCMAWARE_MPI_DIMZ")
            amdgpuaware_MPI[3] = (parse(Int64, ENV["IGG_ROCMAWARE_MPI_DIMZ"]) > 0)
        end
    end
    if all(use_polyester)
        if haskey(ENV, "IGG_USE_POLYESTER_DIMX")
            use_polyester[1] = (parse(Int64, ENV["IGG_USE_POLYESTER_DIMX"]) > 0)
        end
        if haskey(ENV, "IGG_USE_POLYESTER_DIMY")
            use_polyester[2] = (parse(Int64, ENV["IGG_USE_POLYESTER_DIMY"]) > 0)
        end
        if haskey(ENV, "IGG_USE_POLYESTER_DIMZ")
            use_polyester[3] = (parse(Int64, ENV["IGG_USE_POLYESTER_DIMZ"]) > 0)
        end
    end
    ...
    if (device_type != DEVICE_TYPE_NONE)
        if (device_type in [DEVICE_TYPE_CUDA, DEVICE_TYPE_AUTO])
            cuda_enabled = cuda_loaded() && cuda_functional()
        end # NOTE: cuda could be enabled/disabled depending on some additional criteria.
        if (device_type in [DEVICE_TYPE_AMDGPU, DEVICE_TYPE_AUTO])
            amdgpu_enabled = amdgpu_loaded() && amdgpu_functional()
        end # NOTE: amdgpu could be enabled/disabled depending on some additional criteria.
    end
    if (any(nxyz .< 1))
        error("Invalid arguments: nx, ny, and nz cannot be less than 1.")
    end
    if (any(dims .< 0))
        error("Invalid arguments: dimx, dimy, and dimz cannot be negative.")
    end
    if (any(periods .∉ ((0, 1),)))
        error("Invalid arguments: periodx, periody, and periodz must be either 0 or 1.")
    end
    if (any(halowidths .< 1))
        error("Invalid arguments: halowidths cannot be less than 1.")
    end
    if (nx == 1)
        error("Invalid arguments: nx can never be 1.")
    end
    if (ny == 1 && nz > 1)
        error("Invalid arguments: ny cannot be 1 if nz is greater than 1.")
    end
    if (any((nxyz .== 1) .& (dims .> 1)))
        error("Incoherent arguments: if nx, ny, or nz is 1, then the corresponding dimx, dimy or dimz must not be set (or set 0 or 1).")
    end
    if (any((nxyz .< 2 .* overlaps .- 1) .& (periods .> 0)))
        error("Incoherent arguments: if nx, ny, or nz is smaller than 2*overlaps[1]-1, 2*overlaps[2]-1 or 2*overlaps[3]-1, respectively, then the corresponding periodx, periody or periodz must not be set (or set 0).")
    end
    if (any((overlaps .> 0) .& (halowidths .> overlaps .÷ 2)))
        error("Incoherent arguments: if overlap is greater than 0, then halowidth cannot be greater than overlap÷2, in each dimension.")
    end
    ...
    gg = set_global_grid(GlobalGrid(nxyz_g, nxyz, dims, overlaps, halowidths, nprocs, me, coords, neighbors, periods, disp, reorder, comm_cart, cuda_enabled, amdgpu_enabled, cudaaware_MPI, amdgpuaware_MPI, use_polyester, quiet))
    ...
    if (!quiet && me == 0)
        println("Global grid: $(nxyz_g[1])x$(nxyz_g[2])x$(nxyz_g[3]) (nprocs: $nprocs, dims: $(dims[1])x$(dims[2])x$(dims[3]); device support: $support_string)")
    end
    if ((cuda_enabled || amdgpu_enabled) && select_device)
        _select_device()
    end
